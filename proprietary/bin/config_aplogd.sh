...
sqlite3 ${aplogd_db} 'CREATE TABLE config(name TEXT, value TEXT, UNIQUE(name) ON CONFLICT REPLACE)' 2>/dev/null
sqlite3 ${aplogd_db} 'PRAGMA user_version=1;' 2>/dev/null
...
do
    ...
    then
        ...
        sqlite_cmd="INSERT INTO config VALUES(\"$tmp_name\",\"$tmp_value\")"
